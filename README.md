#  DevSpace - Full-stack Blog Application (Lab 2-5)
...
##  Live Demo (Лабораторна №5)
Застосунок розгорнуто у хмарі як Docker-контейнер за допомогою платформи Render:
 **[Відкрити DevSpace Blog](https://devspace-fastapi-blog.onrender.com/)**
...
### 1. Автоматизоване тестування (Pytest)
- Написані **Unit та Інтеграційні тести** для перевірки реєстрації, авторизації та створення/читання постів.
- Використовується ізольована тестова база даних SQLite (`test_app.db`), щоб не впливати на production-дані.
- **Code Coverage:** понад 80% коду покрито тестами (перевищує вимогу в 30%).

### 2. Performance-тестування (Locust)
- Реалізовано складний сценарій: Віртуальний користувач реєструється ➔ проходить авторизацію ➔ отримує JWT токен ➔ створює новий пост.
- **Результати навантаження:** 50 одночасних віртуальних користувачів успішно згенерували понад 1200 запитів без жодної помилки (0% Failures). Скріншоти додано до репозиторію.
![Locust Load Test Results](screenshots/locust_result.png)
...
- Застосунок запаковано у контейнер. Налаштовано `Dockerfile` на базі оптимізованого образу `python:3.10-slim`.
- Налаштовано `.dockerignore` для уникнення потрапляння віртуального середовища та локальних баз даних у контейнер.

### 2. CI/CD Пайплайн
- Налаштовано автоматизований **Continuous Deployment (CD)** через інтеграцію GitHub та хмарної платформи Render.
- При кожному новому коміті (push) у гілку `main`, хмарний сервер автоматично стягує код, перезбирає Docker-контейнер та виконує деплой (Zero-downtime deployment).
